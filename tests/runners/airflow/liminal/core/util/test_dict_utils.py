...
class TestDictUtils(TestCase):
    ...
    def test_merge_dicts(self):
        ...
        self.assertEqual(expected, dict_util.merge_dicts(self.dict1, self.dict2, False))
    ...
    def test_merge_with_empty(self):
        ...
        self.assertEqual(self.dict2, dict_util.merge_dicts({}, self.dict2, False))
    ...
    def test_replace_variables_flat_replace(self, airflow_variable_mock):
        airflow_variable_mock.set("playground", "liminal playground")
        dct = {
        ...
        expected = {'query': 'select * from my_table where event_type = subscription and region = us_east_1',
    ...
    def test_replace_variables_from_env(self):
        dct = {
            "query": "select * from my_table where event_type = {{event_type}} and region = {{region}} from {{table}}"
        }

        variables = {}

        expected = {'query': 'select * from my_table where event_type = {{event_type}} and region '
                             '= {{region}} from my_table'}
    ...
    def test_replace_variables_from_airflow_and_not_enc(self, airflow_variable_mock):
        airflow_variable_mock.set("table", "my_airflow_table")
