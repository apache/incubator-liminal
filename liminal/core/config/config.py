...
class ConfigUtil:
    ...
    def __merge_configs(self, subliminal, superliminal):
        if not superliminal:
            return subliminal
        ...
        if self.__is_subliminal(sub):
            return self.__merge_sub_and_super(sub, merged_superliminal)
        else:
    ...
    def __get_superliminal(self, liminal):
        ...
        if not self.__is_hyperliminal(liminal):
            ...
            if not superliminal_name:
                ...
                if not superliminal:
                    raise FileNotFoundError(f"superliminal '{superliminal_name}' is missing from '{self.configs_path}'")
    ...
    def __merge_sub_and_super(self, sub, supr):
        ...
        return default_configs.apply_variable_substitution(sub, supr)
