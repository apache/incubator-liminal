...
class PythonServerImageBuilder(BasePythonImageBuilder, ServiceImageBuilderMixin):
    ...
    def _additional_files_from_paths():
        python_image_files = os.path.join(os.path.dirname(__file__), '../../image/python')
        return [
            os.path.join(python_image_files, 'container-setup.sh'),
            os.path.join(python_image_files, 'container-teardown.sh'),
